...
> **更新**: 2026-03-13 週五 4pm — 新增 cluster 實戰補強、demo 穩定性策略、fallback 機制
...
## Slurm Quick Start（新增 v4）

> Cluster 的 login node 不能跑 GPU workload。所有 GPU 操作必須透過 Slurm。

```bash
# SSH 到 login node
ssh <username>@<login-node-ip>

# 開 interactive GPU session（hackathon 最適合模式）
srun --gpus=1 --cpus-per-gpu=8 --mem=64G --time=02:00:00 --pty bash
...
**注意**：
- `srun` session 斷線 = GPU 釋放。用 `tmux` 或 `screen` 保護
- `--time` 設長一點（比賽全程），避免 job 被殺
- 如果 Slurm 不可用（直接 SSH 到 GPU node），跳過此步驟

---

## SSH Port Forwarding（Dashboard 訪問，新增 v4）

> Cluster nodes 沒有 browser，dashboard 必須透過 SSH port forwarding 訪問。

```bash
# 在本機開 tunnel（不是在 cluster 上）
ssh -L 8501:localhost:8501 -L 8001:localhost:8001 <username>@<login-node-ip>
```
...
**如果是多跳（login → compute）**：
```bash
# 先 login → compute 的 tunnel
ssh -L 8501:localhost:8501 <compute-node>

# 再本機 → login 的 tunnel
ssh -L 8501:localhost:8501 <login-node>
```

---

## GPU Isolation 檢查（新增 v4）

> Power attribution 假設 GPU 只被本系統使用。若存在其他 workload，功耗數據會失真。
...
```
IF 只有 vLLM process（或無 process）:
│   ✅ GPU 乾淨，繼續
│
IF 有其他 process:
    ⚠️ 功耗數據會被汙染
    → 等待其他 job 結束
    → 或重新申請 exclusive GPU: srun --exclusive ...
    → 或在 demo 時說明：「shared GPU 環境下精度會降低，
       exclusive mode 是生產環境的建議配置」
```
...
nvidia-smi pmon -c 1                      # GPU isolation 檢查（v4 新增）
```

- vLLM 沒在跑 → **先啟動 vLLM**，這是一切的前提
- GPU 有其他 process → 等待或重新申請 exclusive GPU
...
IF ratio > 3x:
│   ✅ 假設成立，原定 demo 方向繼續
│   Demo 論點②：「Decode tokens consume significantly more GPU energy
│     than prefill tokens. On this GPU we measured Xx.」
│
IF 1x < ratio ≤ 3x:
│   ⚠️ 比例存在但偏低
│   → 調整話術：
│     "On a 4060 Ti we measured 8.3x. On this GPU we measured Xx.
│      Different GPU architectures yield different ratios —
│      that's exactly why calibration is needed, not assumptions."
│
IF ratio ≈ 1x:
    🔴 假設不成立，切換備案 B
    → 核心訊息改成：
...
nvidia-smi pmon -c 1                        # GPU isolation（v4 新增）
```
...
| 2 | Decode tokens consume significantly more GPU energy than prefill tokens. On this GPU we measured **___x**. | 週六 calibrate ratio |
...
## GPU Architecture Variability（新增 v4）

> 不同 GPU 架構有不同的 compute/memory 特性。Prefill (compute-bound) 和 decode (memory-bound) 的功耗比例因 GPU 而異。

**4060 Ti 實測數據**（參考）：

| 模型 | W_DECODE (J/tok) | 趨勢 |
|------|------------------|------|
| 0.5B | 0.2435 | 最低 |
| 1.5B | 0.8825 | |
| 3B   | 1.7873 | 最高 |
...
> "On a 4060 Ti we measured 8.3x for Qwen 3B.
>  On this GPU we measured ___x.
>  Each GPU architecture requires its own calibration —
>  that's the product."

**目的**：ratio 不管是多少，都能成為 PowerDecode 價值的佐證 — calibration 本身就是產品。

---

## Claude API Fallback（新增 v4）

> Demo 不依賴即時 LLM call。AI Insight 是加分項，不是必要條件。

```
IF Claude API 正常:
│   ✅ 點按鈕，3 秒出分析
│
IF Claude API timeout / rate limit / no key:
    → Dashboard 其他功能正常（Overview、Detail、Trend）
    → AI Insight 按鈕會顯示錯誤訊息，不影響其他頁面
    → Demo 時跳過 AI 分析，用口述代替：
      "在生產環境中，AI 會自動分析異常模式和定價建議。
       由於網路限制，我們用預先生成的分析結果展示。"
```

**如果需要預錄結果**：週六跑完 seed_demo_data 後，點一次 AI Insight，截圖備用。

---

## Ultimate Fallback Demo（新增 v4）

> 如果 cluster 完全故障（GPU 掛了、Slurm down、SSH 斷了），使用備份 DB 做靜態 demo。
...
**展示內容**：
- Dashboard 三頁全功能（Overview、Detail、Trend）
- 使用真實推理數據（非假資料）
- AI Insight 可用（本機有 API key）

**Demo 說法**：
> "This dashboard is showing cached real inference traces
>  from our calibration runs. In production, this updates in real-time."

**注意**：本機需要先跑過 `restore_local.sh` 確認 DB 正常。週六 Step 4 的備份流程就是為此準備的。
...
| GPU 有其他 process | `nvidia-smi pmon`，等待或 `srun --exclusive` | Demo 說明 shared GPU 限制 |
| Claude API 掛了 | 跳過 AI Insight，口述分析 | 用截圖展示預錄結果 |
| Cluster 完全故障 | `restore_local.sh` + 本機 dashboard | 靜態 demo 用真實數據 |
| Slurm 不給 GPU | 聯繫主辦方 | 用本機 4060 Ti demo |
...
# Ultimate fallback: local-only demo with cached data
bash cluster/restore_local.sh ~/pdd_demo_backup.db
streamlit run dashboard.py
```
